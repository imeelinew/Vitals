...
github_repo="${VITALS_GITHUB_REPOSITORY:-imeelinew/Vitals}"
team_id="${VITALS_TEAM_ID:-5Q5QT76MJU}"
work_dir=$(mktemp -d /tmp/vitals-release.XXXXXX)
...
git add Vitals.xcodeproj/project.pbxproj
if ! git diff --cached --quiet; then
    git commit -m "Release $tag"
fi

archive_path="$work_dir/Vitals.xcarchive"
...
app_path="$archive_path/Products/Applications/Vitals.app"
[[ -d "$app_path" ]] || {
...
archive_file="$work_dir/Vitals-${version}.zip"
